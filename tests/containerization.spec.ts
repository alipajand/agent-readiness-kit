...
describe('checkContainerization', () => {
  ...
  it('awards points for Dockerfile', async () => {
    ...
    expect(result.findings.some((f) => f.message.includes('Dockerfile'))).toBe(
      true,
    );
  });
  ...
  it('awards points for docker-compose.yml', async () => {
    ...
    const result = await checkContainerization(repoPath);
    expect(result.findings.some((f) => f.message.includes('Compose'))).toBe(
      true,
    );
  });
  ...
  it('awards points for .dockerignore', async () => {
    ...
    const result = await checkContainerization(repoPath);
    expect(
      result.findings.some((f) => f.message.includes('.dockerignore')),
    ).toBe(true);
