...
describe('checkNavigability', () => {
  ...
  it('scores above 0 with ROUTES, API, SCORING docs and src modules', async () => {
    ...
    for (const dir of ['src/audit', 'src/generate', 'src/report', 'src/config', 'src/fs']) {
    ...
    expect(result.findings.some((f) => f.message.includes('docs/ROUTES.md'))).toBe(true);
    expect(result.findings.some((f) => f.message.includes('CLI/module directories'))).toBe(true);
  });

  it('adds points for secondary SCHEMA and DATA_MODEL docs', async () => {
    await mkdir(path.join(repoPath, 'docs'), { recursive: true });
    await writeFile(path.join(repoPath, 'docs/SCHEMA.md'), '# Schema\n');
    await writeFile(path.join(repoPath, 'docs/DATA_MODEL.md'), '# Data model\n');
  ...
  it('caps score at 10', async () => {
    ...
    await writeFile(path.join(repoPath, 'docs/SCHEMA.md'), '# Schema\n');
    await writeFile(path.join(repoPath, 'docs/DATA_MODEL.md'), '# Data model\n');
    await writeFile(path.join(repoPath, 'docs/openapi.yaml'), 'openapi: 3.0.0\n');
