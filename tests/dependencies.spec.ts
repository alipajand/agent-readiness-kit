...
describe('checkDependencies', () => {
  ...
  it('awards points for pnpm-lock.yaml', async () => {
    ...
    expect(
      result.findings.some((f) => f.message.includes('pnpm-lock.yaml')),
    ).toBe(true);
  });
  ...
  it('awards points for .nvmrc', async () => {
    ...
    expect(result.findings.some((f) => f.message.includes('.nvmrc'))).toBe(
      true,
    );
  });
  ...
  it('awards points for dependabot.yml', async () => {
    ...
    expect(result.findings.some((f) => f.message.includes('Dependabot'))).toBe(
      true,
    );
  });
  ...
  it('awards points for engines field in package.json', async () => {
    ...
    const result = await checkDependencies(repoPath);
    expect(result.findings.some((f) => f.message.includes('engines'))).toBe(
      true,
    );
  });
