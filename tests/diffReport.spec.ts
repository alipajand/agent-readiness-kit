...
function makeAuditJson(score: number, catScores: Record<string, number>): AuditJson {
  return {
    repoPath: '/tmp/test',
    score,
...
describe('computeDiff', () => {
  ...
  it('identifies new categories', () => {
    const before = makeAuditJson(60, { 'agent-instructions': 10 });
    const after = makeAuditJson(70, { 'agent-instructions': 10, 'dependencies': 8 });
    const diff = computeDiff(before, after);
  ...
  it('identifies removed categories', () => {
    const before = makeAuditJson(70, { 'agent-instructions': 10, 'old-check': 5 });
